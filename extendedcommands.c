...
int show_install_update_menu() {
    ...
    static const char* headers[] = { "Chose zip file", "", NULL };
    ...
    install_menu_items[FIXED_TOP_INSTALL_ZIP_MENUS + num_extra_volumes] = "choose zip from last chosed folder";
    install_menu_items[FIXED_TOP_INSTALL_ZIP_MENUS + num_extra_volumes + 1] = "toggle signature verification";
...
int multiboot_patch(const char* packagefilepath) {
    char rel_path[500];
    extern int popen_wait_done(char *what);

    ui_print("\n-- Processing: %s\n", packagefilepath);

    sprintf(rel_path, "/data/media/0/bootmenu/multiboot.sh %s ; exit $?", packagefilepath);
    
    int status = popen_wait_done(rel_path);
    ui_reset_progress();
    if (status != 0) {
        ui_set_background(BACKGROUND_ICON_ERROR);
        ui_print("Paching unsucesfull!\n");
        return 1;
    }
...
void show_choose_zip_menu(const char *mount_point) {
    ...
    static const char* headers[] = { "Choose a zip you want to patch", "", NULL };

    char* file = choose_file_menu(mount_point, ".zip", headers);
    if (file == NULL)
        return;
    static char* confirm_patch = "Confirm patch?";
    static char confirm[PATH_MAX];
    sprintf(confirm, "Yes - patch %s", basename(file));

    if (confirm_selection(confirm_patch, confirm)) {
        //install_zip(file);
	multiboot_patch(file);
